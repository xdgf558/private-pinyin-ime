...
- 支持基础上下文预测，目前 bigram 数据仍较小，预测能力后续还会继续扩充。
...
用户词库只用于改善本地排序、一步联想和短句补全。它不是云同步，也不会自动读取剪贴板或应用上下文。当前学习能力包括
「候选选择记忆」「已选词转移记忆」和「短句补全记忆」：你经常选的词会逐渐排得更靠前，常在某个词后面选择的下一个词也会出现在联想候选里；如果你反复输入类似 `今天`、`天气`、`不错` 的连续候选，下次提交 `今天` 后也可以直接联想出 `天气不错`。复杂的整句自动切词和大规模预测还在后续规划中。
