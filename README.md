...
- iOS 键盘内可在「简体」与「繁體」输出之间切换；转换完全在本机完成，候选、预测和提交字形保持一致，共用同一套本地学习排序。
- iOS 候选条每组提供最多 9 个候选，长词可横向滑动查看，翻组按钮始终固定可见。
- iOS 数字符号页提供 `#+=` 入口，可输入括号、书名号、运算符、省略号等更多特殊符号，并可用 `123` 返回基础符号。
