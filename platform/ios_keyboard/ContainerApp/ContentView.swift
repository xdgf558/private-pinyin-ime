            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 9) {
                    Text("本次更新")
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundStyle(StationTheme.textPrimary)
                    Label("进一步稳定 X 发布与发送时的键盘收起动画，阻止延迟候选和高度刷新拉动宿主界面", systemImage: "rectangle.compress.vertical")
                    Label("快速九宫格输入只合并已过期的界面帧，所有按键仍在共享核心中按顺序执行", systemImage: "speedometer")
                    Label("异步输入尚未显示时，退格仍作用于拼音组合，不会误删输入框原有正文", systemImage: "delete.backward.fill")
                    Label("扩大符号页句号、斜线等边缘按键的有效点击区域，并让字段切换后的旧候选明确变灰", systemImage: "hand.tap.fill")
                }
                .font(.system(size: 13))
                .foregroundStyle(StationTheme.textSecondary)
                .padding(16)
