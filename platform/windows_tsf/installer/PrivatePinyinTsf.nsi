...
Section "PrivatePinyin IME" SecMain
  ...
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\PrivatePinyin" "DisplayIcon" "$INSTDIR\private-pinyin-settings.exe"
  ...
  CreateShortcut "$SMPROGRAMS\PrivatePinyin IME\Setup Guide.lnk" "$WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe" "-NoProfile -ExecutionPolicy Bypass -File $\"$INSTDIR\open-onboarding.ps1$\""
  CreateShortcut "$SMPROGRAMS\PrivatePinyin IME\Preferences.lnk" "$WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe" "-NoProfile -ExecutionPolicy Bypass -File $\"$INSTDIR\open-settings.ps1$\""
