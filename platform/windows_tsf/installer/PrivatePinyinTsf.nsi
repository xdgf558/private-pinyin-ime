...
Section "猫栈拼音" SecMain
  ...
  File "${PACKAGE_SOURCE}\PrivatePinyinAIHelper.exe"
  File "${PACKAGE_SOURCE}\open-writer.ps1"
  ...
  SetOutPath "$INSTDIR\${APP_DIR_NAME}\WriterRuntime"
  File /r "${PACKAGE_SOURCE}\WriterRuntime\*"
  SetOutPath "$INSTDIR\${APP_DIR_NAME}"
  ...
  CreateShortcut "$SMPROGRAMS\猫栈拼音\本地 Writer.lnk" "$WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe" "-NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -STA -File $\"$INSTDIR\${APP_DIR_NAME}\open-writer.ps1$\"" "$INSTDIR\${APP_DIR_NAME}\PrivatePinyinInstaller.ico"
  ...
  Delete /REBOOTOK "$INSTDIR\PrivatePinyinAIHelper.exe"
  ...
  Delete /REBOOTOK "$INSTDIR\open-writer.ps1"
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\猫栈拼音\本地 Writer.lnk"
