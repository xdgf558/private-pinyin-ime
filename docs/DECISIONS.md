...
Date: 2026-07-28
Status: accepted
Decision: Keep recording every eligible local selection in the existing decaying SQLite user lexicon, but require three confirmations before a learned word or context identity contributes ranking weight. Treat the first two decayed observations as warm-up data, subtract that warm-up allowance from mature weight, and apply the resulting effective weight consistently to direct candidates, bigram/trigram/short-phrase predictions, continuous transitions, and AI Lite learning features.
...
Consequences: macOS, Windows, full-key iOS, and nine-key iOS inherit the same policy through the shared core and existing `用户学习` switch. Confirmations one and two remain stored, exportable, capacity-managed local history but receive no user-source or context boost; confirmation three starts with the former one-selection influence. The existing 30-day half-life applies before the threshold, so an old mature preference can naturally fall back to warm-up while its raw history remains available for later reinforcement. Existing rows require sufficient current decayed weight and are not force-promoted during upgrade. Strict privacy and disabled learning still write nothing. AI Lite may consume only effective learning weight, preventing optional reranking from bypassing the threshold. No input, candidate, context, or learning content is logged or sent over a network.
