...
Consequences: Reviewed rules may apply even when the raw spelling has a valid but commonly confused parse; generic edits run only when the raw input has no complete parse, require lowercase ASCII without apostrophes, stop after 24 characters, and remain bounded to 64 generated checks and two accepted candidates. Corrected candidates appear at the low-priority tail of the first visible page and can be reranked by the already-approved AI Lite typo feature, but AI absence or disablement leaves deterministic base behavior. No raw input, correction, or candidate text is logged or persisted by this feature. Nine-key results remain byte-for-byte unchanged and require a separate `NINEKEY-TYPO-01` design with digit-signature-specific quality and latency evidence.
