...
Consequences: macOS, Windows, and iOS expose one `宽容拼音` master control; turning it on enables all seven reviewed pairs, while existing hand-edited per-pair settings remain intact until the master control is explicitly changed. Five-candidate pages expose at most one combined typo/tolerant correction and wider pages at most two. Candidate metadata is marked probable for optional AI Lite ordering, but AI absence retains deterministic local behavior. Strict privacy may use this stateless path because it performs no network request, learning write, content log, or alternate-spelling persistence. Input longer than 24 characters, non-ASCII input, incomplete syllable parses, multiple simultaneous fuzzy transformations, and all nine-key digit input are excluded. Exact-only indexed lookup avoids rerunning the continuous lattice; the Apple-host regression retains the existing 60-ms per-key budget.
