...
Consequences: Enter commits raw composition, Escape cancels without commit, Backspace recomputes one step and can replay the same candidate identities, paging changes only the visible page, and Chinese comma/period use the same default identity as Space before appending punctuation. Each accepted action produces at most one commit. Desktop hosts continue to pass idle digits and composition-scoped control keys to applications; iOS uses candidate taps and expanded-grid controls instead of physical numbered selection, with its in-flight guard preventing duplicate commits. Candidate one retains ABC-01's fixed-snapshot stability. Positions two through nine are guaranteed to stay aligned with their displayed identity after a page is visible, but are intentionally not permanent across changed context, settings, learning snapshots, or a fresh pre-display AI result. ABC-02 and ABC-03 do not gain alternate interaction paths. No typed text, candidate identity, or acceptance content is logged or persisted by this policy.
