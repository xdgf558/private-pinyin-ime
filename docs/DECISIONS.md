...
Consequences: The bundled base grows to 137,699 entries using MIT phrase-pinyin-data at a low supplemental frequency. rime-ice and other GPL dictionaries are never packaged by the project; users may import their own copies and remain responsible for upstream terms. Imports require explicit pinyin rows, use atomic writes, survive upgrades, and can be cleared without touching bundled or learned data. macOS and Windows use a 64-MiB source, 128-MiB canonical-file, and 750,000-entry policy for larger desktop dictionaries. iOS retains the original 16-MiB source, 32-MiB canonical-file, and 200,000-entry policy so desktop expansion cannot widen the Keyboard Extension memory boundary. On iOS, the container App invokes the same Rust importer while it owns security-scoped access and writes the App Group layer; the keyboard extension is a read-only consumer, preserving the no-Full-Access posture. App Group failure disables importing without affecting typing.
...
Consequences: The checked-in report contains only metrics, result codes, output lengths, artifact identity, and the decision; prompts and generated text remain absent. The offline probe's fixed synthetic prompts may be passed to llama.cpp through argv only inside this evaluation tool; production Writer content must use the authenticated AI-09 Helper protocol and never enter process arguments. The current first-byte measurement includes fresh process startup and cold model loading. This candidate cannot enter the AI-05 approval registry, application bundle, installer, Helper, or product UI. AI-11 is blocked on selecting a stronger candidate that passes a new exact-hash evaluation, adds native Windows peak-RSS evidence plus separate cold/warm latency evidence, and receives separate Owner redistribution approval. Quality gates must not be weakened merely to convert this result to Go.

## Decision 041: Gate AI-11 Writer Contracts and Keep the Stronger Candidate Unshipped

Date: 2026-07-21
Status: accepted
Decision: Add a versioned, bounded Writer request/preview protocol to the authenticated AI-09 Helper and evaluate the exact Apache-2.0 Qwen2.5 1.5B Instruct Q4_K_M candidate at revision `dd26da440ef0330c47919d1ecae0966d24022222` with official llama.cpp release `b10069`. Keep all Writer settings default-off and keep the model unapproved, unregistered, unbundled, and disconnected from every platform host. A valid Writer frame currently receives only the content-free `ModelUnavailable` result.
Reason: The stronger candidate passed all five first-party synthetic completion, rewrite, and translation cases on the development Mac. Cold-process first-byte latency was 321-444 ms, cancellation completed immediately, and measured peak RSS was about 1,192 MiB. This establishes technical promise but is not cross-platform release evidence: warmed-request latency and native Windows RSS are still missing, and the Owner has not approved redistribution of the exact artifact.
Consequences: Production Writer content may travel only over authenticated helper IPC, never through argv, environment variables, temporary files, logs, telemetry, or persistent caches. Requests are capped at 4,096 UTF-8 bytes and 600 characters, return at most three bounded previews, expire within three seconds, and carry complete session/request/revision/source identity. Rewrite and translation require explicit actions; any eventual replacement requires user confirmation. Short completion requires a separate default-off opt-in whose UI says that pausing sends the current composition to the local AI process. Strict privacy force-disables all three content-bearing Writer modes while allowing stateless AI Lite reranking to follow its separate policy. Late, cancelled, malformed, oversized, mismatched, or unavailable-model results leave user text unchanged. Writer UI and inference remain unavailable until quality, license, Owner, Mac, and Windows gates all pass.

## Decision 042: Ship the AI-12 Lite Profile While Writer Remains No-Go

Date: 2026-07-22
Status: accepted
Decision: Complete AI-12 against the releasable profile that contains the approved AI Lite ranker and the dormant Writer protocol, while explicitly retaining `NoGo` for Writer inference. Make the distinction machine-readable and fail CI if model notices, categorized privacy fixtures, exact AI-off equivalence, or bounded Helper fault contracts disappear.
Reason: AI-12 is a hardening and release-decision stage, not permission to weaken AI-11's model gates. The existing Lite path has an approved 426-byte coefficient package and bounded fail-soft host integration. Writer has promising Mac quality evidence but still lacks warmed-request data, native Windows RSS, signed-package Helper identity smokes, and exact Owner redistribution approval. Treating the whole AI track as blocked would withhold proven Lite improvements; treating Writer as complete would misrepresent its evidence and privacy boundary.
Consequences: First-party synthetic privacy fixtures cover passwords, tokens, identity cards, phone numbers, payment cards, and expected false positives. Desktop and iOS feature builds prove privacy-blocked AI output exactly matches the base engine. Helper tests cover maximum frames, oversized rejection, queue saturation, cancellation, deadlines, bounded restart attempts, crash recovery, shutdown, and idle exit, with Windows platform behavior compiled and executed in CI. Releases may include AI Lite and the dormant helper/protocol, but no Writer weights, inference, UI, or host request path. Writer activation remains blocked by `AI-OI-004`, `AI-OI-010`, and `AI-OI-011`; those gates require new evidence and a separate reviewed decision rather than a silent change to this profile.

## Decision 043: On-Demand Verified Desktop Writer Runtime

Date: 2026-07-22
Status: accepted
Decision: Enable an explicit-action Writer V1 on macOS arm64 and Windows x64 by packaging the exact MIT-licensed llama.cpp `b10069` runtime while keeping the Apache-2.0 Qwen2.5 1.5B Instruct Q4_K_M model outside every app bundle and installer. The user must separately consent to downloading the exact model from its fixed official Hugging Face revision; the host and Helper verify its `1,117,320,736`-byte size and SHA-256 `6a1a2eb6d15622bf3c96857206351ba97e1af16c30d7a74ee38970e434e9407e` before use.
Reason: The Owner explicitly approved local, user-initiated use of the exact AI-11 candidate after its first-party quality evaluation, but did not approve redistribution of the one-gigabyte model. On-demand installation keeps the normal package small and preserves that license boundary. The AI-09 process boundary keeps model loading, memory, cancellation, and crashes outside IMK and TSF while allowing verified offline generation.
Consequences: V1 exposes only user-triggered rewrite and Chinese/English translation previews; results are copied or selected explicitly and never replace text automatically. Automatic pause-triggered short completion remains disabled. Strict privacy, revoked consent, removed model state, request cancellation, setting changes during inference, timeout, malformed IPC, runtime failure, or model verification failure discards the result and leaves ordinary typing unchanged. Content travels only through authenticated AI-09 IPC and a separately authenticated loopback connection to the Helper-owned `llama-server`; it never enters argv, logs, telemetry, or temporary prompt files. The AI-09 Helper token is never reused outside Helper IPC. Each server launch receives a fresh independent 256-bit key through a private key file rather than argv; the file uses mode `0600` on Unix and the current-user application-data ACL on Windows, and the Helper removes it as soon as authenticated readiness succeeds. The runtime starts with `--offline`, `--no-webui`, and disabled logs, and the model download URL is fixed in host code rather than derived from input. The pinned non-streaming HTTP response contract, loopback port reserve/rebind retry, and one-request-at-a-time runtime lock remain bounded fail-closed implementation constraints. Public release still requires the signed-package macOS and Windows smoke matrix in `AI-OI-010` and `AI-OI-012`.

## Decision 044: Reviewed Opt-In White Frost Desktop Layer

Date: 2026-07-24
Status: accepted
Decision: Offer macOS and Windows users an explicit GPL-3.0 consent flow that downloads only the Owner-reviewed White Frost `rime-frost` 1.0.4 stable Release from its official GitHub repository, verifies its exact byte count and SHA-256, parses a fixed six-file dictionary allowlist from the ZIP without extracting it, and atomically writes a separate `rime_frost.tsv` layer.
Reason: White Frost provides useful broad desktop coverage, but its GPL-3.0 data must not become part of the all-rights-reserved application package or the permissive base lexicon. A user-initiated, reproducible import can preserve that license boundary while avoiding moving branches, unofficial mirrors, arbitrary archive extraction, and silent replacement of other local data.
Consequences: Version `1.0.4` is pinned to the official 44,008,360-byte `rime-frost-schemas.zip` asset with SHA-256 `4f4998ae83f63d757c0a4ace192f69d48265bddfabe231642b73e3739ed0f2f5`. The request starts at the fixed official GitHub Release URL and follows HTTPS redirects without pinning GitHub's changeable asset-CDN hostnames; exact size and SHA-256 remain the authoritative identity. The importer rejects an unexpected artifact identity, duplicate or unsafe member name, symlink or special member, excessive entry count, excessive expanded/member size, or excessive compression ratio before replacing the previous layer. Only `8105`, `41448`, `base`, `ext`, `others`, and `corrections` dictionaries are retained. The White Frost layer is independent from the bundled base, manual Rime imports, reviewed Rime Ice data, and learned SQLite data; it can be enabled, disabled, updated, or cleared alone. Generic desktop Rime imports use the expanded 64-MiB source policy, while the reviewed White Frost ZIP applies a tighter 32-MiB limit to each approved member; both use the 128-MiB canonical-file and 750,000-entry desktop limits. The archive verifier and its ZIP/SHA dependency chain are behind a desktop-only Cargo feature, and macOS performs verification and parsing on a dedicated queue with a short-lived engine so the IMK main thread and shared typing engine remain available. iOS remains at 16 MiB, 32 MiB, and 200,000 entries and has no White Frost download path or ZIP parser. A newer upstream release is displayed only as `新版待审核`; download/import stays pinned until the Owner reviews and updates the recorded artifact identity.

## Decision 045: Bounded Full-Keyboard Typo Candidates Before Nine-Key Correction

Date: 2026-07-26
Status: accepted
Decision: Implement TYPO-01 in the shared Rust full-keyboard candidate path using the reviewed AI-04 correction table plus bounded duplicate-key removal, adjacent QWERTY-key substitution, and neighboring-letter transposition. Validate every corrected spelling through the existing pinyin parser and production lexicon, add at most two correction candidates, retain the raw composition and every original candidate, and expose exact/probable/weak metadata to AI Lite instead of replacing text automatically.
Reason: Full-keyboard typos have a clear physical-key adjacency model and can be corrected consistently on macOS, Windows, and iOS without creating platform-specific behavior. Unrestricted edit distance would generate too many false positives and repeated lexicon work, while rewriting the composition would make recovery difficult. Nine-key input has an ambiguous digit-to-letter model and a separate incremental lattice, so treating it as QWERTY correction would be incorrect.
Consequences: Reviewed rules may apply even when the raw spelling has a valid but commonly confused parse; generic edits run only when the raw input has no complete parse, require lowercase ASCII without apostrophes, stop after 24 characters, and remain bounded to 64 parser attempts and two accepted candidates. Five-item pages expose at most one correction so four ordinary paths remain visible; wider pages may expose two. macOS, Windows, and iOS provide an independent setting that defaults on. Strict privacy still permits this stateless local correction path while disabling learning, statistics, and content-bearing Writer actions. Corrected candidates can be reranked by the already-approved AI Lite typo feature, but AI absence or disablement leaves deterministic base behavior. No raw input, correction, or candidate text is logged or persisted by this feature. Nine-key results remain byte-for-byte unchanged and require a separate `NINEKEY-TYPO-01` design with digit-signature-specific quality and latency evidence.

## Decision 046: Bounded Nine-Key Typo Candidates After Ordinary Decoding

Date: 2026-07-27
Status: accepted
Decision: Add NINEKEY-TYPO-01 as a bounded postpass after the ordinary exact, continuous, and prefix nine-key decoder. Generate only one-edit alternate digit signatures for a missing digit, extra digit, adjacent keypad substitution, or adjacent transposition; require an exact lexicon identity; append no more than two correction candidates; and preserve the raw digit composition plus every ordinary candidate in its original order.
Reason: Nine-key signatures are intentionally ambiguous, so QWERTY adjacency and unrestricted edit distance would create frequent false positives and repeat expensive lattice work. A postpass keeps correction outside the session-local incremental lattice, reuses the compact exact-key index for viability checks, and lets the ordinary decoder remain the authority. When the typed signature already has an exact reading, single-character corrections are rejected so a valid ambiguous signature such as `636` is not displaced merely because `626` can spell `mao`.
Consequences: Correction accepts only `2` through `9`, stops after 24 digits, spends at most 64 viability attempts, and returns at most two candidates carrying exact/probable/weak metadata for the existing AI Lite typo feature. Compact five-item pages may expose one correction and wider pages two, while all ordinary candidates remain reachable. The existing default-on `拼音智能纠错` setting controls both QWERTY and nine-key correction on macOS, Windows, and iOS; disabling it restores the exact pre-stage output. Strict privacy may still use this stateless local path because no composition, corrected signature, or candidate text is logged, persisted, or sent over a network. Apple-host regressions retain the unchanged 60-ms per-key budget and compare correction-on/off at both a 21-key sentence and the 64-key maximum.

## Decision 047: Stable Default Candidate Identity

Date: 2026-07-27
Status: accepted
Decision: Treat the first candidate produced by the deterministic base engine as the stable Space-key default for the current visible page. Optional AI Lite output must be an exact permutation, may reorder only candidates after that stable prefix, and must fail closed without changing the page if its identity, text, index set, revision, or deadline is invalid.
Reason: Fast habitual input depends on Space selecting the same default regardless of whether an asynchronous optional ranker happens to finish before the page is shown. Letting timing move candidate three into candidate one makes identical typing produce different text even though the base engine and learning state are unchanged. Locking the first identity protects blind typing while retaining useful AI improvement among the remaining candidates.
Consequences: ABC-01 is implemented once in the shared Rust core and therefore applies to macOS, Windows, and iOS AI Lite integrations. The public session reorder API rejects any exact permutation that would move the current page's first candidate, so a future caller cannot bypass the default-identity invariant. New raw input, context, settings, or learning data still triggers a complete fresh base ranking; stability means that the same composition, context, and learning snapshot reproduces the same ordered candidate identities, not that stale candidates survive changed input. Backspace followed by retyping the same suffix restores the same full candidate order. Paging does not mutate identities, and selection always commits the currently displayed candidate exactly once. Positions 2 through 9 are intentionally not cross-run blind-typing guarantees: optional AI Lite may reorder them before the page is first displayed, while AI-07 keeps every already visible numbered page frozen. Pinning the whole page would make optional reranking a no-op, so users receive a stable Space default and post-display numeric-key stability rather than permanent lower-slot identities. No candidate text, pinyin, or input is logged or persisted by the stability policy.

## Decision 048: Bounded Opt-In Syllable-Level Tolerant Pinyin

Date: 2026-07-27
Status: accepted
Decision: Implement ABC-02 as a default-off full-keyboard postpass in the shared Rust core. When explicitly enabled, the reviewed bidirectional `zh/z`, `ch/c`, `sh/s`, `n/l`, `an/ang`, `en/eng`, and `in/ing` pairs may change exactly one complete legal syllable per alternate path. Generate at most 16 alternate spellings, query only exact base and user-lexicon identities, add at most two low-priority candidates, and never rewrite the raw composition or remove/reorder an ordinary candidate.
Reason: Regional pronunciation and typing habits should not force users to remember a rigid spelling, but unrestricted fuzzy parsing would multiply paths, hide precise results, and make fast habitual Space commits unpredictable. A bounded exact-lookup postpass keeps the ordinary parser and continuous lattice authoritative while reusing an already present settings schema instead of introducing platform-specific rules.
Consequences: macOS, Windows, and iOS expose one `宽容拼音` master control; turning it on enables all seven reviewed pairs, while existing hand-edited per-pair settings remain intact until the master control is explicitly changed. Five-candidate pages expose at most one combined typo/tolerant correction and wider pages at most two. TYPO-01 candidates enter first and consume that visible correction allowance before ABC-02; tolerant candidates fill only any remaining allowance. Candidate metadata is marked probable for optional AI Lite ordering, but AI absence retains deterministic local behavior. Strict privacy may use this stateless path because it performs no network request, learning write, content log, or alternate-spelling persistence. Input longer than 24 characters, non-ASCII input, incomplete syllable parses, multiple simultaneous fuzzy transformations, and all nine-key digit input are excluded. Exact-only indexed lookup avoids rerunning the continuous lattice; the Apple-host regression retains the existing 60-ms per-key budget.

## Decision 049: Three-Confirmation Gentle Local Learning

Date: 2026-07-28
Status: accepted
Decision: Keep recording every eligible local selection in the existing decaying SQLite user lexicon, but require decayed weight 3.0 before a learned word or context identity first contributes ranking weight. Persist a maturity bit beside each direct, bigram, trigram, and short-phrase learning identity. Once mature, keep the identity active through the 2.0-3.0 band; deactivate it only below 2.0, after which it must reach 3.0 again. A 0.001 numerical tolerance absorbs only the minute decay accumulated while three immediate confirmations cross separate SQLite writes; it remains far too small for two confirmations to activate learning. Subtract the two-observation warm-up allowance from rankable weight and apply the result consistently to direct candidates, predictions, continuous transitions, and AI Lite learning features.
Reason: A one-off correction, accidental tap, or topic-specific choice should not immediately replace a familiar Space-key default or dominate the next prediction. Waiting for three confirmations distinguishes a repeat habit from an isolated event without adding user friction, a second database, host-specific state, or permanent thresholds that ignore time decay.
Consequences: macOS, Windows, full-key iOS, and nine-key iOS inherit the same policy through the shared core and existing `用户学习` switch. Confirmations one and two remain stored, exportable, capacity-managed local history but receive no user-source or context boost; reaching 3.0 starts with the former one-selection influence. The 3.0 activation / 2.0 deactivation hysteresis prevents a moderately used preference from repeatedly changing the Space-key default while its weight decays through the boundary. After it falls below 2.0, later confirmations accumulate from the retained decayed weight until 3.0 is reached again. During upgrade, the schema migration marks only existing rows whose current decayed weight is at least 3.0 as mature. Consequently, a user's one-off and two-use long tail can stop affecting candidate order immediately after upgrading, while its data remains available for reinforcement; sufficiently recent repeated habits continue to influence ranking. This one-time visible reset is intentional and is covered by upgrade smoke tests. Strict privacy and disabled learning still write nothing. AI Lite may consume only rankable learning weight, preventing optional reranking from bypassing the state boundary. No input, candidate, context, or learning content is logged or sent over a network.

## Decision 050: Shared Blind-Typing Key Contract and Three-Platform Acceptance

Date: 2026-07-28
Status: accepted
Decision: Close ABC-04 by making Space and numbered selection explicit shared-core policies, exercising the complete blind-typing recovery sequence through Rust and C ABI tests, and requiring equivalent macOS, Windows, and iOS release smokes. Space commits visible candidate one for an active composition, or raw input when no candidate exists; without a composition it remains a literal space even if prediction candidates are visible. Keys `1` through `9` select only the matching slot on the current visible composition page. Without a composition, desktop physical keys and iOS on-screen number keys insert literal digits even when prediction candidates are visible; predictions require an explicit candidate click or tap. An unavailable composition-page slot fails closed without changing composition or candidate identity.
Reason: Candidate quality alone does not make an input method comfortable to use without watching it. Habitual input depends on a small keyboard grammar staying invariant across host applications, optional AI timing, paging, correction, tolerant input, learning, and nine-key mode. Keeping that grammar in the shared session prevents three platform hosts from slowly developing different Space, number, Enter, Escape, Backspace, or page semantics.
Consequences: Enter commits raw composition, Escape cancels without commit, Backspace recomputes one step and can replay the same candidate identities, paging changes only the visible page, and Chinese comma/period use the same named default identity as Space before appending punctuation. Each accepted action produces at most one commit. All hosts distinguish active composition from prediction-only state so `你好2` is never converted into a prediction selection or swallowed. iOS candidate taps and expanded-grid controls remain explicit selection paths, while its on-screen number page follows the same composition-only numbered-selection rule and its in-flight guard prevents duplicate commits. Candidate one retains ABC-01's fixed-snapshot stability. Positions two through nine are guaranteed to stay aligned with their displayed identity after a page is visible, but are intentionally not permanent across changed context, settings, learning snapshots, or a fresh pre-display AI result. ABC-02 and ABC-03 do not gain alternate interaction paths. No typed text, candidate identity, or acceptance content is logged or persisted by this policy.

## Decision 051: iOS Haptic Feedback and the Full Access Boundary

Date: 2026-07-29
Status: accepted
Decision: Keep the shipping Keyboard Extension at `RequestsOpenAccess=false` and do not promise physical haptic feedback. Existing view-attached UIKit feedback calls remain fail-soft compatibility code: input behavior, candidate selection, and key responsiveness must never depend on tactile output. Enabling Full Access solely to obtain haptics is outside the scope of the inset-row hit-target fix and requires a separate Owner-approved privacy decision, explicit user-facing disclosure, and a new physical-device acceptance matrix.
Reason: Full Access enlarges the custom keyboard sandbox and materially changes the user's privacy expectations. Tactile feedback is useful polish, but it does not justify silently weakening the project's established no-Full-Access default or making a release check impossible under the shipping configuration.
Consequences: TestFlight reviewers validate first-tap key accuracy and the absence of dropped or delayed input, not the presence of vibration. Missing haptics must not be attributed to the device's Sounds & Haptics settings for the current build. The source may retain `UIFeedbackGenerator` setup because it is harmless when unavailable and preserves a reviewed implementation if the privacy policy changes later; it must remain optional and content-free. Any future Full Access proposal must explain the additional capabilities it unlocks, keep cloud/network behavior independently disabled unless separately approved, and provide a clear opt-in path before physical haptics become a product claim.
