...
Consequences: AI-02 adds feature/hardware/budget contracts, redacted content-bearing debug output, sanitized error codes, deadline handling, identity validation, and a mock provider only. It does not change input behavior or touch the engine, FFI, or platform hosts. AI-03 must add PrivacyGuard and minimal-context enforcement; AI-07 must supply trustworthy host revisions and discard responses whose complete identity is no longer current.
