...
Date: 2026-07-28
Status: accepted
Decision: Close ABC-04 by making Space and physical number selection explicit shared-core policies, exercising the complete blind-typing recovery sequence through Rust and C ABI tests, and requiring equivalent macOS, Windows, and iOS release smokes. Space commits visible candidate one for an active composition, or raw input when no candidate exists; without a composition it remains a literal space even if prediction candidates are visible. Keys `1` through `9` select only the matching slot on the current visible composition page. Without a composition, desktop hosts pass digits through even when prediction candidates are visible; predictions require an explicit candidate click or tap. An unavailable composition-page slot fails closed without changing composition or candidate identity.
...
Consequences: Enter commits raw composition, Escape cancels without commit, Backspace recomputes one step and can replay the same candidate identities, paging changes only the visible page, and Chinese comma/period use the same named default identity as Space before appending punctuation. Each accepted action produces at most one commit. Desktop hosts distinguish active composition from prediction-only state so `你好2` is never converted into a prediction selection or swallowed. iOS uses candidate taps and expanded-grid controls instead of physical numbered selection, with its in-flight guard preventing duplicate commits. Candidate one retains ABC-01's fixed-snapshot stability. Positions two through nine are guaranteed to stay aligned with their displayed identity after a page is visible, but are intentionally not permanent across changed context, settings, learning snapshots, or a fresh pre-display AI result. ABC-02 and ABC-03 do not gain alternate interaction paths. No typed text, candidate identity, or acceptance content is logged or persisted by this policy.
