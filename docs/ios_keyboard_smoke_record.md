...
| Same-App field-switch review closure | pending simulator rerun | The debug host now exposes two fields. Switching focus without dismissing the keyboard must clear the first field's candidates on the next main-loop turn, while a transition that reaches `disappearing` remains frozen |
