...
| Callback regression | passed | Synchronous self callbacks and matching non-empty delayed callbacks are consumed; external clear/send, empty/nil/old contexts, expiry, unavailable UUID, and replaced document identities fail closed. Beginning a new operation after expiry also discards stale evidence |
| Document identity | passed | DEBUG Simulator logs showed the proxy `ObjectIdentifier` remained unchanged across attachment callbacks, confirming it is not a document identity. The nullable C bridge returned nil during attachment and a UUID once available without triggering Swift's unconditional bridge trap |
| Stable portrait geometry | passed | An actual rendered-surface probe measured QWERTY, nine-key, symbols, and expanded candidates at `view=278` and `root=258`, all with `clipped=false`. Minimum visible button heights were 44/44/44/49 points. The DEBUG probe now terminates the smoke run if clipping appears or any visible button drops below 44 points |
...
| Same-App field switch | passed | Focus moved directly to the second field without dismissing while the first field still showed a nine-key composition. The first delivered `6` replaced the stale surface immediately; deleting that composition to empty and typing `6` again restored live candidates without deleting host text. A fresh `64426` then committed exactly one `你好` in the second field |
| X-like send/dismiss | passed (equivalent host transition) | `模拟发送并收起` cleared both host fields and dismissed on the next main-loop turn. A 100-ms frame sample showed one monotonic dismissal with no reverse movement or second geometry pull. Reopening the first field restored the custom keyboard, and a second `64426` committed exactly one `你好` |
...
| Same-App field-switch review closure | pending simulator rerun | The debug host now exposes two fields. Switching focus without dismissing the keyboard must clear the first field's candidates after the bounded 50 ms settling window, while a transition that reaches `disappearing` remains frozen |
...
| Physical-device retest | pending | Repeat X Publish/Send with `0.1.29` or the next TestFlight candidate, then return to the same input field and verify live candidates. With `RequestsOpenAccess=false`, haptic output is not an acceptance requirement; verify only that absent tactile feedback never delays or drops a key. If host pulling remains, capture the relative timing of `textDidChange`, `viewWillDisappear`, and queued core-reset completion; a disappearance later than the 50 ms settling window is the first residual hypothesis |
