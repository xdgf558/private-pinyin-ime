...
| Physical-device retest | pending | Repeat X Publish/Send with `0.1.29` or the next TestFlight candidate, then return to the same input field and verify live candidates plus physical haptic feedback. If host pulling remains, capture the relative timing of `textDidChange`, `viewWillDisappear`, and queued core-reset completion; a disappearance later than the 50 ms settling window is the first residual hypothesis |
