...
| Stable portrait geometry | passed | An actual rendered-surface probe measured QWERTY, nine-key, symbols, and expanded candidates at `view=278` and `root=258`, all with `clipped=false`. Minimum visible button heights were 44/44/44/49 points, so the unified portrait height does not shrink keys below 44 points |
