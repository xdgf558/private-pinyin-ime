...
1. 以规范化后的原始拼音字符位置构建词语 lattice，不先固定唯一音节切分。
2. 使用有界 Beam Search 同时选择拼音边界和词语边界。
3. 路径分数由对数词频、基础 bigram、本地用户 bigram 和短语长度因子组成。
4. 撇号必须对应候选词拼音中的合法音节边界。
5. 整句候选保留内部词段；用户提交后，仅在学习开启且非严格隐私模式时记录相邻词转移。
6. 单次会话最多加载 5,000 条本地用户转移用于内存评分，SQLite 原始数据仍按现有保留策略管理。
