...
| Host submit dismissal | In X or an equivalent social compose screen, type and commit a nine-key phrase, then immediately tap Publish/Send. The first external document-change callback freezes the final rendered geometry and no timer may thaw it before dismissal; the host view moves only with its own keyboard animation and is not pulled a second time by a late candidate, height, key-layout, or core-reset refresh. Return to an input field using the warm extension, enter `64426`, and confirm the first key recovers the surface, the candidate bar updates to `你好`, and it commits exactly once | | Repeat with compact candidates visible, after paging in the expanded view, and after switching from QWERTY to nine-key. Also switch directly between two fields without dismissing; the first key in the second field must clear the old visual state and appear exactly once |
| Immediate response | Rapid full-key and nine-key taps keep pace with touch input. Rapidly enter, Backspace, and re-enter `64426`: all core events execute in order, superseded intermediate nine-key UIKit frames may be coalesced, and the final preedit/candidate order matches slow entry exactly | | With Full Access off, unavailable haptic calls must add no per-key work. QWERTY typing, empty-composition Backspace, and punctuation remain unchanged |
