...
| Self-change callback | Commit several candidates quickly and switch host fields within 250 ms; delayed `textDidChange` callbacks caused by the keyboard's own insert/delete operations do not clear the new candidate or prediction state, while a nil/expired/old context or changed document-proxy identity from a real external field fails closed and resets composition without crashing | | |
| Visible field switch | With the keyboard kept onscreen, type enough pinyin to show candidates in the first diagnostic field, then tap the second field without dismissing. Old controls must become inert while geometry remains fixed; typing `64426` in the second field replaces them with a fresh `你好`. Repeat while dismissing the keyboard and confirm the disappearing surface does not thaw or change height | | |
