...
| Mouse candidate selection | In TextEdit, Safari, Chrome, and VS Code, type `dabaoyige` with a lexicon containing `打包一个`, click that multi-character candidate, and confirm it commits exactly once. Repeat after rapidly extending and deleting the composition; a candidate from an old panel generation must neither commit nor clear a newer selection | | Also click two candidates with identical visible text when available. Empty or attribute-stripped final IMK callbacks must resolve through a current attributed identity, verified highlight, or generation-bound panel snapshot rather than becoming a no-op. Inspect content-free `candidate_selection_resolved` source events; repeated `candidate_selection_unresolved` events block release. Direct-text compatibility fallback does not count as a learning/context-path success |
