...
| Visible field switch | With the keyboard kept onscreen, type enough pinyin to show candidates in the first diagnostic field, tap the second field without dismissing the keyboard, and wait one main-loop turn. The old preedit and candidates clear before the next key; typing `64426` in the second field produces a fresh `你好`. Repeat while dismissing the keyboard and confirm the disappearing surface does not thaw or change height | | |
