...
| ABC-04 blind typing | In Notepad and one Chromium/Electron app, verify `nihao` + Space commits `你好` once; current-page keys `1` through `9` commit the matching visible identity, while an unavailable slot preserves composition. PageDown + `2` selects page-two candidate two; Enter commits raw pinyin, Esc cancels, Backspace/retype restores order, and comma uses the same default as Space | | Repeat with AI Lite on/off, correction on/off, prediction visible, and after ABC-03 learning changes. Idle digits/arrows/control keys must still pass through |
...
| ABC-04 blind typing | In Notes and X or another social compose host, verify QWERTY `nihao` + Space and nine-key `64426` + Space each commit `你好` once. Tap every compact candidate, then use the expanded grid to page and select a visible identity; rapid double taps still commit once. Enter commits raw composition, Backspace/retype restores order, and returning after warm reuse accepts the first key once | | Repeat after extension-process recreation, with AI Lite fallback, correction on/off, prediction visible, and after ABC-03 learning changes. Candidate content must not move after display |
