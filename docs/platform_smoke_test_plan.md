...
| Host submit dismissal | In X or an equivalent social compose screen, type and commit a nine-key phrase, then immediately tap Publish/Send. The keyboard keeps its final rendered frame through dismissal; the host view moves only with the system keyboard animation and is not pulled a second time by a late candidate, height, or key-layout refresh. Return to an input field using the warm extension, enter `64426`, and confirm the candidate bar updates to `你好` and commits exactly once | | Repeat with compact candidates visible, after paging candidates, and after switching from QWERTY to nine-key |
