...
| Host submit dismissal | In X or an equivalent social compose screen, type and commit a nine-key phrase, then immediately tap Publish/Send. The first external document-change callback freezes the final rendered frame before dismissal; the host view moves only with the system keyboard animation and is not pulled a second time by a late candidate, height, key-layout, or delivered-key refresh. Return to an input field using the warm extension, enter `64426`, and confirm the candidate bar updates to `你好` and commits exactly once | | Repeat with compact candidates visible, after paging in the expanded view, and after switching from QWERTY to nine-key. If a second pull remains, first record the relative timing of `textDidChange`, `viewWillDisappear`, and queued core-reset completion to determine whether disappearance exceeded the 50 ms settling window |
