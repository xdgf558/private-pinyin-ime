...
| ABC-02 tolerant input | Enable 宽容拼音, type `zongguo`, and confirm `中国` is reachable without changing the raw preedit or Space-key default. Disable it and confirm the alternate disappears while ordinary candidates remain identical | | Repeat `zh/z`, `n/l`, and `in/ing`; nine-key output must remain unchanged |
...
| ABC-02 tolerant input | Enable 宽容拼音, type `zongguo`, and confirm `中国` is reachable without changing the raw preedit or Space-key default. Disable it and confirm the alternate disappears while ordinary candidates remain identical | | Repeat in TextEdit, Safari, Chrome, and VS Code; verify nine-key output is unchanged |
...
| ABC-02 tolerant input | In QWERTY, enable 宽容拼音 and type `zongguo`; `中国` is reachable while the preedit remains `zongguo` and the ordinary Space-key default is unchanged. Disable it and confirm ordinary output is identical | | Repeat after App/keyboard recreation and verify the setting persists; nine-key mode must remain unchanged |
