...
| ABC-04 blind typing | In Notes and X or another social compose host, verify QWERTY `nihao` + Space and nine-key `64426` + Space each commit `你好` once. Tap every compact prediction candidate rather than treating physical digits as prediction shortcuts, then use the expanded grid to page and select a visible identity; rapid double taps still commit once. Enter commits raw composition, Backspace/retype restores order, and returning after warm reuse accepts the first key once | | Repeat after extension-process recreation, with AI Lite fallback, correction on/off, prediction visible, and after ABC-03 learning changes. Candidate content must not move after display |
