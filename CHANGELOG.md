...
- Fixed the macOS imported-lexicon status line being vertically clipped, added friendly local dictionary names, and recognized `rime-ice`/`雾凇` source folders as `雾凇拼音` for newly recorded imports. Legacy imported layers without metadata now explain that re-importing is required because source paths are intentionally not retained.
