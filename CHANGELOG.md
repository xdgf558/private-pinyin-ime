...
- Closed the remaining physical-device X submission race by freezing the iOS keyboard surface at the first external document-change callback, before host dismissal begins, and by allowing key-event thaw only while the extension is presented. UIKit feedback generators are now attached to the keyboard view, and the compact candidate strip keeps only the downward expanded-candidate entry instead of separate previous/next arrows.
