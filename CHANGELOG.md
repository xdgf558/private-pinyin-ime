...
- Added ABC-04 blind-typing interaction and acceptance across the shared Rust core and all three platform hosts. Space, visible-page number selection, Enter, Escape, Backspace/retype, paging, punctuation, prediction-state handling, nine-key default commit, and exactly-once selection now have one explicit contract with Rust, C ABI, CI, and release-smoke coverage.
