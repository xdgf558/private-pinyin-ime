...
- Removed the remaining iOS X Publish/Send timing guess: an external document change now keeps keyboard geometry frozen until an actual appearance callback or newly delivered key proves the warm extension is active again, so a host that begins dismissal later than 50 ms cannot receive a late height or candidate-layout refresh. Rapid nine-key digit and Backspace operations still execute in order in the shared core but coalesce superseded UIKit output frames, and the extension skips unavailable feedback-generator work while Full Access remains disabled.
