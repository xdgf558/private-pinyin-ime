...
- Fixed intermittent macOS candidate-panel clicks that appeared to do nothing. Displayed candidates now carry a generation-and-index identity, selection changes retain the exact highlighted item, and an empty or attribute-stripped final InputMethodKit callback can still commit the currently bound phrase exactly once. Stale panel selections fail closed after the candidate list changes.
